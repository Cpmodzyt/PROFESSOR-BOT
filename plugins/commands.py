...
async def start(client, message):
    try:
        ...
        if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
            ...
            await message.reply(START_MESSAGE.format(user=user_name, bot=client.mention), reply_markup=InlineKeyboardMarkup(buttons), disable_web_page_preview=True)                    
            await asyncio.sleep(2) 

            # Check if the chat is in the database, if not add it
        ...
        # Add user to the database if they do not exist
        if not await db.is_user_exist(message.from_user.id):
            await db.add_user(message.from_user.id, message.from_user.first_name)
            await client.send_message(LOG_CHANNEL, script.LOG_TEXT_P.format(message.from_user.id, message.from_user.mention, message.from_user.username, temp.U_NAME))
        
        # If the command is not length 2 (e.g., no arguments), show default message
        if len(message.command) != 2:
            ...
            # Ensure user name is valid
            user_name = message.from_user.mention if message.from_user else message.chat.title
            await message.reply_photo(photo=random.choice(PICS), caption=START_MESSAGE.format(user=user_name, bot=client.mention), reply_markup=InlineKeyboardMarkup(buttons), parse_mode=enums.ParseMode.HTML)
            return
        
        # Force subscribe logic if the user is not subscribed
        ...
        # Handle case where message command is exactly "subscribe", "error", "okay", or "help"
